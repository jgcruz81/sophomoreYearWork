...
void finalhuff(){
    //if root = 0;
    char *byte = malloc(1);
    int fd = open("huff2.txt", O_WRONLY | O_CREAT, S_IRUSR | S_IWUSR);
    char start = '\\';
    char newline = '\n';
    write(fd,&start,1);
    write(fd,&newline,1);
...
void swap(struct MinTree **x) {
    struct MinTree *temp = *x;
    head = temp->next;
    
    struct MinTree *ptr = temp->next;
    while ((ptr->occ < temp->occ) && ptr->next != NULL) {
        ptr = ptr->next;
    }
    if (ptr->next == NULL && (ptr->occ < temp->occ)) {
        ptr->next = temp;
        temp->next = NULL;
        return;
    }
...
void compress(char filename[], char huffman[]){
    ...
    int output = open("use.txt", O_WRONLY | O_CREAT, S_IRUSR | S_IWUSR);
...
void decompress(char decomp[], char huff[]){
    
    char *p = decomp;
    p[strlen(p)-4] = 0;
    printf("%s",p);
    int file = open(decomp, O_RDONLY);
    int output = open(p, O_WRONLY | O_CREAT, S_IRUSR | S_IWUSR);
    int huffman = open(huff, O_RDONLY);
...
int main() {
    //place linked list of structs into root
    
    char str[255];
    memset(&str,'\0',255);
    int file = open("poop.txt", O_RDONLY);
    int j = 0;
    char x;
    /*
    char tester[] = "";
    ...
    */
    ...
    char huffman[] = "huff.txt";
    
    buildhuff();
    compress(filename, huffman);
    
    char decomp[] = "filename.txt.hcz";
    decompress(decomp, huffman);
